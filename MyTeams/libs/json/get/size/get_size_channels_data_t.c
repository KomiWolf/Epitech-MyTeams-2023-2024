...
size_t get_size_channels_data_t(channels_data_t *data, int tab_level)
{
    ...
    total_size += get_node_size((char *)JSON_S_NAME, "channels_data_t", ntl);
    ...
    total_size += JSON_COMMA;
    total_size += strlen("threads") + JSON_COLON + JSON_NEW_LINE + ntl;
    total_size += get_string_size(data->threads, LINKED_LIST, ntl);
    return total_size;
}
